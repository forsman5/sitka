...
These values are configuration constants, not final balance decisions. Migration pressure is the dominant early signal of hardship: a household under sustained pressure is marked as wanting to relocate, and that pressure is visible in settlement/game-over reporting well before anything drastic happens. Pressure alone never moves anyone, though -- early testing showed that acting on it directly (a much looser, earlier signal than starvation) drained workers out of settlements that were merely food-insecure but otherwise stable fast enough that the departures themselves caused the shortage that then finished the settlement off, something that never happened when pressure was reporting-only. Instead, relocation is gated on the same strict condition that would otherwise mean a starvation death: a household about to become a starvation candidate gets one chance to relocate to a connected settlement with meaningfully better rolling grain fulfillment instead of dying there -- keeping its size and wealth, resetting its stress/pressure counters, and settling in for a short cooldown before it can be reported as pressured again. A settlement with no qualifying neighbor (isolated, or every neighbor equally hard-off) still loses that household to starvation as before. Relocation will never take a settlement below COLLAPSE_HOUSEHOLD_THRESHOLD households -- some minimum stays behind regardless, so hardship thins a settlement rather than hollowing it out completely via relocation. Starvation has no such floor: it is the only thing that can still take a settlement all the way to zero if it truly cannot feed the households it has left, including the ones that had nowhere to relocate to. During starvation, reduce household members before deleting an empty household; remove dependents before workers for the initial mechanical model. This is deliberately an aggregate pressure model, not an assertion about historical household behavior.
...
63 households were under sustained migration pressure (12 relocated elsewhere in the valley) and 11 people died.
```
